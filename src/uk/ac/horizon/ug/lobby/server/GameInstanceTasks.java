...
public class GameInstanceTasks implements Constants {
		...
		try {
			...
			try {
				...
				case ACTIVE:
					...
					case ENDED:
					case CANCELLED:
						doEndFromPreparing(gi, factory, server, em);
						break;
					case ENDING:
						doEndingFromActive(gi, factory, server, em);
						break;
				...
				case PLANNED:
					...
					case ENDED:
					case CANCELLED:
						doCancelFromPlanned(gi, factory, server, em);
						break;
					case READY:
						doPreparingFromPlanned(gi, factory, server, em);
						doReadyFromPreparing(gi, factory, server, em);
						break;
					case ACTIVE:
						doPreparingFromPlanned(gi, factory, server, em);
						doReadyFromPreparing(gi, factory, server, em);
						doActiveFromReady(gi, factory, server, em);
						break;
					case ENDING:
						doPreparingFromPlanned(gi, factory, server, em);
						doReadyFromPreparing(gi, factory, server, em);
	...
	private static void updateStatus(GameInstance gi, GameInstanceStatus oldStatus, GameInstanceStatus newStatus, EntityManager em) {
		EntityTransaction et = em.getTransaction();
		// just set the state :-)
		et.rollback();
		et.begin();
	...
	private static void doEndFromPreparing(GameInstance gi,
		...
		updateStatus(gi, null, GameInstanceStatus.ENDED, em);
	}
}
